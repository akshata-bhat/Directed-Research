...
os.environ["CUDA_VISIBLE_DEVICES"] = "" # To ensure the keras framework runs on CPU and not GPU

#Changing the labels in the range 0 - 9 to 0, even label(1), odd_label(2)
def relabel(label):
	if(label == 0):    
		label = 0
	elif(label%2 == 0):
...
def preprocess_labels(Y_train, Y_test):
	size_train = np.shape(Y_train)[0]
	for i in range(size_train):
		Y_train[i] = relabel(Y_train[i])
	size_test = np.shape(Y_test)[0]
	for i in range(size_test):
		Y_test[i] = relabel(Y_test[i])
